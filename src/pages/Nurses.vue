<template>
  <DefaultLayout title="nurses">
    ...
    <AddStaff :type="EStaffType.Nurse" />
    <Staff
      v-for="(n, index) in nurses"
      :key="index"
      :staff="n"
      :index="index + 1"
      :type="EStaffType.Nurse"
    />
  </DefaultLayout>
</template>
...
const nurses = computed(() =>
  $store.state.staffs.filter((s: IStaff) => s.type === EStaffType.Nurse)
);
</script>
